 ...
 *
 * Test clock is fixed at 2026-06-08 (Monday) via global-setup.ts.
 * All rule rejections must come from the SERVER (HTTP 400), not client-side UX.
 */
import { test, expect } from '@playwright/test';
import { VALID_RC, CLINIC_TRAUMA, CLINIC_ANGIOLOGY, TUESDAY, THURSDAY } from './helpers/booking';
...
test('HP1: full booking wizard — urology Mon-Fri, confirms with booking ID', async ({ page }) => {
  ...
  // Confirmation card
  await expect(page.locator('text=Objednávka potvrdená, text=Booking confirmed')).toBeVisible({ timeout: 15_000 });
});
...
test('R1: angiology on Wednesday (not in bookingDays=[4,5]) → 400', async () => {
  const { status, body } = await postBooking({ ...BASE, clinicId: CLINIC_ANGIOLOGY, date: '2026-06-10', time: '13:20' });
...
test('R2: angiology on Thursday 09:00 (outside 13:00–14:00 window) → 400', async () => {
  const { status, body } = await postBooking({ ...BASE, clinicId: CLINIC_ANGIOLOGY, date: THURSDAY, time: '09:00' });
  expect(status).toBe(400);
...
test('R4: closed clinic (diabetologicka) → 400', async () => {
  const { status, body } = await postBooking({ ...BASE, clinicId: 'diabetologicka', date: TUESDAY, time: '09:00' });
  expect(status).toBe(400);
  expect(body.message).toMatch(/bookable|closed|not bookable/i);
});

test('R5: trauma surgery on Monday (bookingDays=[2,4]) → 400', async () => {
  const { status, body } = await postBooking({ ...BASE, clinicId: CLINIC_TRAUMA, date: '2026-06-08', time: '09:00' });
  expect(status).toBe(400);
...
test('R9: past date → 400', async () => {
  const { status, body } = await postBooking({ ...BASE, clinicId: CLINIC_TRAUMA, date: '2026-06-01', time: '09:00' });
  expect(status).toBe(400);
...
test('HP3: cancel page renders for any token; non-existent token shows not-found state', async ({ page }) => {
  ...
  await expect(page.locator('text=Nastala chyba, text=Objednávku sa nepodarilo')).toBeVisible({ timeout: 10_000 });
});
