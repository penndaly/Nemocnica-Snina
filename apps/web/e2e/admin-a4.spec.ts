 ...
 * lanes: they skip when the API is unreachable or TEST_SUPERADMIN_JWT is absent
 * (a real super_admin staff access token, StaffJwtGuard-verifiable — see
 * global-setup.ts; CMS_AUTH_BYPASS=true also works in dev). This is
 * deliberately a separate fixture from TEST_STAFF_JWT (a 'clinician'-role
 * token for a different auth system, used by telehealth.spec.ts) — reusing
 * one identity across role tiers would stop exercising the real
 * super_admin-only role check these endpoints enforce.
 *
...
const SUPERADMIN_JWT = process.env['TEST_SUPERADMIN_JWT'];
const auth = () => ({ Authorization: `Bearer ${SUPERADMIN_JWT}` });
...
test.describe('A4-3 — platform toggle compliance blocks', () => {
  test('enabling a partnership platform → 422 partnership_required', async ({ request }) => {
    test.skip(!SUPERADMIN_JWT, 'needs TEST_SUPERADMIN_JWT');
...
test.describe('A4-6 — global threshold update writes a global-scope audit entry', () => {
  test('PUT thresholds/defaults validates + persists', async ({ request }) => {
    test.skip(!SUPERADMIN_JWT, 'needs TEST_SUPERADMIN_JWT (administrator+)');
