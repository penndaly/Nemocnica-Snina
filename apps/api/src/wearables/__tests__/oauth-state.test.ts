...
describe('OAuthStateService — Redis-backed CSRF hardening (WL9 Part A / WR-6)', () => {
  ...
  it('accepts a freshly issued, platform-matched state once', async () => {
    ...
    expect(await svc.consumeState(state, 'fitbit')).toEqual({ patientToken: 'tok-1' });
  });

  it('rejects a reused state (one-time use, atomic GETDEL) → 400', async () => {
    const svc = make();
    const state = await svc.generateState('tok-1', 'fitbit');
    await svc.consumeState(state, 'fitbit');
    await expect(svc.consumeState(state, 'fitbit')).rejects.toThrow(BadRequestException);
  });
  ...
  it('rejects an expired state → 400 INVALID_OAUTH_STATE', async () => {
    ...
    await expect(svc.consumeState(state, 'fitbit')).rejects.toThrow('INVALID_OAUTH_STATE');
  });

  it('rejects a forged / unknown state → 400 INVALID_OAUTH_STATE', async () => {
    const svc = make();
    await expect(svc.consumeState('deadbeef.forged', 'fitbit')).rejects.toThrow('INVALID_OAUTH_STATE');
  });
  ...
  it('rejects a tampered HMAC → 400 (before any Redis read)', async () => {
    ...
    await expect(svc.consumeState(`${nonce}.${'0'.repeat(64)}`, 'fitbit')).rejects.toThrow(BadRequestException);
  });
  ...
  it('rejects a state issued for another platform → STATE_PLATFORM_MISMATCH', async () => {
    ...
    await expect(svc.consumeState(state, 'fitbit')).rejects.toThrow('STATE_PLATFORM_MISMATCH');
  });
  ...
  it('a state is not accepted by an instance that never stored it (cross-instance isolation)', async () => {
    ...
    await expect(b.consumeState(state, 'fitbit')).rejects.toThrow(BadRequestException);
  });
