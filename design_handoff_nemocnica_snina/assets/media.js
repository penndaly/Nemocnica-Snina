...
(function () {
  ...
  /* ---- Hero: full-bleed image behind the hero copy, with a contrast scrim ---- */
  function mountHero(o) {
    const hero = document.querySelector(".page-hero") || document.querySelector(".hero-wrap");
    if (!hero || hero.querySelector(".hero-media")) return;
    hero.classList.add("has-hero-media");
    hero.insertAdjacentHTML("afterbegin",
      `<div class="hero-media"><image-slot id="${at(o.id)}" shape="rect" fit="cover" placeholder="${at(tx(o.label))}"></image-slot></div><div class="hero-scrim"></div>`);
  }
  ...
  function mountInsert(o) {
    ...
    let html;
    if (o.hero) {
      html = `<section class="media-hero-band"><div class="media-frame" style="aspect-ratio:${o.ratio || "12/5"}"><image-slot id="${at(o.id)}" shape="rect" fit="cover" placeholder="${at(tx(o.label))}"></image-slot></div></section>`;
  ...
  function upgradeExisting(page) {
    ...
    document.querySelectorAll(".doc-card .avatar").forEach((av) => {
      ...
      const ini = (av.textContent || "").trim();
      av.innerHTML = `<image-slot id="doc-${at(id)}" shape="circle" fit="cover" data-initials="${at(ini)}" placeholder="${at(tx(P("Portrét", "Portrait")))}"></image-slot>`;
    });
  }
  ...
  const PLAN = {
    "index.html": {
      hero: { id: "home-campus", label: P("Hlavná budova Nemocnice Snina — široká fotografia areálu", "Nemocnica Snina main building — wide campus photo") },
      inserts: [
    ...
    "pre-pacientov.html": {
      ...
      inserts: [
        ...
        { sel: "#podakovania .container", pos: "beforeend", id: "patients-discharge", ratio: "12/5",
  ...
  function run() {
    ...
    if (!plan) return;
    let heroPlan = plan.hero;
    if (plan.heroDept) {
      const did = new URLSearchParams(location.search).get("id");
      heroPlan = { id: did ? "dept-" + did + "-hero" : "departments-hero",
        label: P("Fotografia oddelenia", "Department photo") };
    }
    if (heroPlan) mountHero(heroPlan);
    (plan.inserts || []).forEach(mountInsert);
    /* dev guard: every declared slot must actually exist in the DOM */
    const declared = [];
    if (heroPlan) declared.push(heroPlan.id);
