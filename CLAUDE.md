...
- **The "booking date tests are timezone-sensitive" flakiness was a real, fixed bug, not infra noise.** Every date-string helper in `booking-api.e2e.test.ts`/`booking-edge-cases.e2e.test.ts`/`booking-rules.test.ts` picked a weekday with local `.getDay()` but serialized it with `.toISOString()` (UTC) — in any UTC+ timezone, local midnight is still the previous day in UTC, so the returned date's actual weekday didn't match. Fixed by building date strings from local components everywhere (`nextAvailableDates()` in `booking-rules.service.ts` already did this correctly — that was the template). Don't reintroduce `.toISOString().substring(0,10)` for a date whose weekday was checked via local `getDay()`.
- **Verification claims name the method, not the outcome.** Write "content present + scroll ladder + computed style", never "renders correctly". Sprint ROUTE-1b shipped `/pre-pacientov` with five layout classes that had no rule in `globals.css`; content-presence and the no-scroll ladder both passed (inert grids stack, and stacking *reduces* width). The ladder now has two mandatory rungs for any new/ported markup: `pnpm --filter=@ns/web audit:classes` (static: every `className` must resolve to a rule in `packages/ui/src/globals.css` or the script's allowlist — runs in CI lint) and a `getComputedStyle` row in `apps/web/e2e/styled.spec.ts` for every class you port (runtime: defined *and* applied). `globals.css` is the only stylesheet; Tailwind utilities are emitted but are not the convention — new ones must be allowlisted by name. See `docs/03-AUDIT.md` "UI-2a".
- `*.e2e`/`*.db` suites still need a live Postgres for the "real DB" test blocks (`Cases 8+10`, `audit-immutability.db.test.ts`) — that's a legitimate infra requirement, not a bug. `@nestjs/platform-express` is installed as an `apps/api` devDependency (needed by `@nestjs/testing`'s `createNestApplication()`, which defaults to Express unless a platform adapter is passed) — don't remove it. Telehealth/LiveKit suites are unverified as of this note; confirm a failure is pre-existing (`git stash` your change) before chasing it.
